...
function TripDetail({ trip, tab, setTab, updateTrip, removeTrip, toast, cloudReady, readOnly = false, provider = 'deepseek', workspaceId = '', user = null }) {
  ...
  const regenerateSection = async (section) => {
    ...
    try {
      ...
      const result = await generateWithAi({ provider, workspaceId, action: "regenerate", section, brief, currentSection, lockedItems });
      ...
      toast(`Bagian ${section === "activities" ? "rundown" : section === "travelGuide" ? "panduan" : section} berhasil diregenerasi. Item terkunci tetap dipertahankan.`);
    } catch (error) {
  ...
  return <div className={`detail-view${readOnly ? ' read-only-detail' : ''}`}>
    ...
    {readOnly && <div className="read-only-banner" role="status">Mode viewer · perubahan dinonaktifkan</div>}
      ...
      <div className="detail-actions"><button className="light-button" onClick={() => setModal({ type: 'trip', item: trip })}>Edit detail</button><button className="light-button" onClick={() => window.print()}>Cetak / PDF</button>{!readOnly && <select className="ai-regenerate-select" aria-label="Regenerasi bagian dengan AI" value="" onChange={(event) => { if (event.target.value) regenerateSection(event.target.value); }} disabled={!workspaceId || Boolean(regeneratingSection)}><option value="">Regenerasi AI…</option><option value="activities">Rundown</option><option value="tasks">Checklist</option><option value="expenses">Anggaran</option><option value="documents">Dokumen</option><option value="risks">Risiko</option><option value="travelGuide">Panduan</option><option value="facts">Fakta</option><option value="assumptions">Asumsi</option><option value="verificationNotes">Verifikasi</option><option value="conflicts">Konflik</option><option value="alternatives">Alternatif</option></select>}{regeneratingSection && <span className="generation-inline-status">Menyusun {regeneratingSection}…</span>}<button className="danger-light" onClick={() => removeTrip(trip)}>Hapus</button></div>
    </section>
    <div className="print-options" aria-label="Pilihan format cetak"><span>Export:</span><button type="button" onClick={() => printAudience('participant')}>Peserta</button><button type="button" onClick={() => printAudience('organizer')}>Panitia</button></div>
